...
void	smoke_update_emitter(void *g, t_emitter *emitter)
{
	...
	while (index < MAX_PARTICLES)
	{
		if (emitter->particles[index].active)
		{
			...
			draw_rect(game->screen, c_rect((int)emitter->particles[index].x, \
...
t_emitter	*create_smoke_emitter(t_game *g, int x, int y, int loop)
{
	...
	index = 0;
	obj = (t_emitter *)ft_malloc(sizeof(t_emitter));
	if (!obj)
		return (NULL);
	obj->x = x;
	obj->y = y;
	...
	obj->live = 0;
	obj->max_live = 1.0f;
	obj->particles = (t_particle *) \
	ft_malloc(sizeof(t_particle) * (MAX_PARTICLES + 1));
	while (index < MAX_PARTICLES)
	{
